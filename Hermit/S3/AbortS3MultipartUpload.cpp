...
#include "AbortS3MultipartUpload.h"
...
namespace hermit {
	namespace s3 {
		...
		//
		S3Result AbortS3MultipartUpload(const HermitPtr& h_,
			...
			while (true) {
				...
				if (result.mStatus == S3Result::k307TemporaryRedirect) {
					if (result.mNewEndpoint.empty()) {
						NOTIFY_ERROR(h_,
									 "AbortS3MultipartUpload: S3Result::k307TemporaryRedirect but new endpoint is empty for url:",
									 url);
						return S3Result::kError;
					}
					if (result.mNewEndpoint == host) {
						NOTIFY_ERROR(h_,
									 "AbortS3MultipartUpload: S3Result::k307TemporaryRedirect but new endpoint is the same for url:",
									 url);
						return S3Result::kError;
					}
					if (++redirectCount >= 5) {
						NOTIFY_ERROR(h_, "AbortS3MultipartUpload: too many temporary redirects for url:", url);
						return S3Result::kError;
					}
				...
				if (result.mStatus != S3Result::kSuccess) {
					NOTIFY_ERROR(h_, "AbortS3MultipartUpload: SendS3Command failed for URL:", url);
					return S3Result::kError;
				}
