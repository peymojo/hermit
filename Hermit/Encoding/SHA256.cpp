...
#define E38_SHA256_K(I) SHA256_round_constants[I]
			...
			void SHA256ProcessBlock(SHA256State& ioState,
									const void* inBuffer,
									uint64_t inDataSize) {
...
H = t0 + t1; \
		...
		//
		void SHA256ProcessBytes(SHA256State& ioState,
								const void* inData,
								uint64_t inDataSize) {
		...
		//
		static void SHA256Conclude(SHA256State& ioState) {
		...
		//
		void* CalculateSHA256(const char* inData, uint64_t inDataSize, void* outResult) {
			SHA256State state;
			SHA256Init(state);
		...
		//
		void* CalculateSHA224(const char* inData, uint64_t inDataSize, void* outResult) {
			SHA256State state;
			SHA224Init(state);
			SHA256ProcessBytes(state, inData, inDataSize);
